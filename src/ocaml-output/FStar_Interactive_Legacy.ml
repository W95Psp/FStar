...
[@@deriving show]
type stack_t = (env_t * modul_t) Prims.list[@@deriving show]
...
type interactive_state =
  {
  chunk: FStar_Compiler_Util.string_builder ;
  stdin:
    FStar_Compiler_Util.stream_reader FStar_Pervasives_Native.option
...
let (__proj__Mkinteractive_state__item__chunk :
  interactive_state -> FStar_Compiler_Util.string_builder) =
  fun projectee ->
...
let (__proj__Mkinteractive_state__item__buffer :
  interactive_state -> input_chunks Prims.list FStar_Compiler_Effect.ref) =
  fun projectee ->
...
let (the_interactive_state : interactive_state) =
  let uu___ = FStar_Compiler_Util.new_string_builder () in
  let uu___1 = FStar_Compiler_Util.mk_ref FStar_Pervasives_Native.None in
  let uu___2 = FStar_Compiler_Util.mk_ref [] in
  let uu___3 = FStar_Compiler_Util.mk_ref FStar_Pervasives_Native.None in
...
let rec (read_chunk : unit -> input_chunks) =
  fun uu___ ->
    ...
    let log =
      ...
      then
        let transcript =
          let uu___2 = FStar_Compiler_Effect.op_Bang s.log in
          match uu___2 with
          | FStar_Pervasives_Native.Some transcript1 -> transcript1
          | FStar_Pervasives_Native.None ->
              let transcript1 =
                FStar_Compiler_Util.open_file_for_writing "transcript" in
              (FStar_Compiler_Effect.op_Colon_Equals s.log
    ...
    let stdin =
      ...
      | FStar_Pervasives_Native.None ->
          let i = FStar_Compiler_Util.open_stdin () in
          (FStar_Compiler_Effect.op_Colon_Equals s.stdin
             (FStar_Pervasives_Native.Some i);
           i) in
    let line =
      let uu___1 = FStar_Compiler_Util.read_line stdin in
      match uu___1 with
      | FStar_Pervasives_Native.None ->
          FStar_Compiler_Effect.exit Prims.int_zero
      | FStar_Pervasives_Native.Some l -> l in
    log line;
    (let l = FStar_Compiler_Util.trim_string line in
     if FStar_Compiler_Util.starts_with l "#end"
     then
       let responses =
         match FStar_Compiler_Util.split l " " with
         | uu___2::ok::fail::[] -> (ok, fail)
         | uu___2 -> ("ok", "fail") in
       let str = FStar_Compiler_Util.string_of_string_builder s.chunk in
       (FStar_Compiler_Util.clear_string_builder s.chunk;
        Code (str, responses))
     else
       if FStar_Compiler_Util.starts_with l "#pop"
       then (FStar_Compiler_Util.clear_string_builder s.chunk; Pop l)
       else
         if FStar_Compiler_Util.starts_with l "#push"
         then
           (FStar_Compiler_Util.clear_string_builder s.chunk;
            (let lc_lax =
               let uu___5 =
             ...
             let lc =
               ...
               | l1::c::"#lax"::[] ->
                   let uu___5 = FStar_Compiler_Util.int_of_string l1 in
           ...
           if FStar_Compiler_Util.starts_with l "#info "
           then
             (match FStar_Compiler_Util.split l " " with
              | uu___5::symbol::[] ->
                  (FStar_Compiler_Util.clear_string_builder s.chunk;
              ...
              | uu___5::symbol::file::row::col::[] ->
                  (FStar_Compiler_Util.clear_string_builder s.chunk;
                   (let uu___7 =
                      let uu___8 =
                        let uu___9 =
                          let uu___10 = FStar_Compiler_Util.int_of_string row in
                          let uu___11 = FStar_Compiler_Util.int_of_string col in
             ...
             then
               ...
               if l = "#finish"
               then FStar_Compiler_Effect.exit Prims.int_zero
               else
                 (FStar_Compiler_Util.string_builder_append s.chunk line;
                  FStar_Compiler_Util.string_builder_append s.chunk "\n";
                  read_chunk ()))
let (shift_chunk : unit -> input_chunks) =
  fun uu___ ->
    let s = the_interactive_state in
    let uu___1 = FStar_Compiler_Effect.op_Bang s.buffer in
    match uu___1 with
    | [] -> read_chunk ()
    | chunk::chunks ->
        (FStar_Compiler_Effect.op_Colon_Equals s.buffer chunks; chunk)
let (fill_buffer : unit -> unit) =
  fun uu___ ->
    let s = the_interactive_state in
    let uu___1 =
      let uu___2 = FStar_Compiler_Effect.op_Bang s.buffer in
      let uu___3 = let uu___4 = read_chunk () in [uu___4] in
      FStar_Compiler_List.op_At uu___2 uu___3 in
    FStar_Compiler_Effect.op_Colon_Equals s.buffer uu___1
...
type m_timestamps =
  (Prims.string FStar_Pervasives_Native.option * Prims.string *
    ...
    FStar_Compiler_Util.time) Prims.list[@@deriving show]
let rec (tc_deps :
  modul_t ->
    stack_t ->
      FStar_TypeChecker_Env.env ->
        ...
        fun env ->
          fun ts ->
            let is_stale intf impl intf_t impl_t =
              ...
              (FStar_Compiler_Util.is_before impl_t impl_mt) ||
                 ...
                 | (FStar_Pervasives_Native.Some intf1,
                    FStar_Pervasives_Native.Some intf_t1) ->
                     let intf_mt =
                       FStar_Parser_ParseIt.get_file_last_modification_time
                         intf1 in
                     FStar_Compiler_Util.is_before intf_t1 intf_mt
                 | (FStar_Pervasives_Native.None,
            ...
            let rec iterate depnames st env' ts1 good_stack good_ts =
              ...
              let rec pop_tc_and_stack env1 stack ts2 =
                ...
                | uu___::ts3 ->
                    (pop env1 "";
                     (let uu___2 =
                        let uu___3 = FStar_Compiler_List.hd stack in
                        let uu___4 = FStar_Compiler_List.tl stack in
                        (uu___3, uu___4) in
                      match uu___2 with
                      | ((env2, uu___3), stack1) ->
                            ...
                            then
                              let env1 =
                                pop_tc_and_stack env'
                                  (FStar_Compiler_List.rev_append st []) ts1 in
                              tc_deps m good_stack env1 depnames good_ts
                            else
                              (let uu___4 =
                                 let uu___5 = FStar_Compiler_List.hd st in
                                 let uu___6 = FStar_Compiler_List.tl st in
                                 (uu___5, uu___6) in
                               match uu___4 with
                               | (stack_elt, st') ->
            ...
            | (filenames, uu___1, dep_graph) ->
                iterate filenames (FStar_Compiler_List.rev_append stk []) env
                  (FStar_Compiler_List.rev_append ts []) [] []
let (format_info :
  FStar_TypeChecker_Env.env ->
    Prims.string ->
          ...
          fun doc ->
            ...
            let uu___2 =
              ...
              | FStar_Pervasives_Native.Some docstring ->
                  FStar_Compiler_Util.format1 "#doc %s" docstring
              | FStar_Pervasives_Native.None -> "" in
            FStar_Compiler_Util.format4 "(defined at %s) %s: %s%s" uu___ name
              uu___1 uu___2
let rec (go :
  (Prims.int * Prims.int) ->
          ...
          fun env ->
            fun ts ->
                  ...
                  let info_opt =
                    ...
                    | FStar_Pervasives_Native.None ->
                        ...
                        then FStar_Pervasives_Native.None
                          ...
                          (let lid =
                             let uu___2 =
                               FStar_Compiler_List.map FStar_Ident.id_of_text
                                 (FStar_Compiler_Util.split symbol ".") in
              ...
              | Completions search_term ->
                  let rec measure_anchored_match search_term1 candidate =
                    ...
                    | (hs::ts1, hc::tc) ->
                        ...
                        if FStar_Compiler_Util.starts_with hc_text hs
                        then
                          (match ts1 with
                           | [] ->
                    ...
                    FStar_Compiler_List.iter
                      (fun uu___3 ->
                         ...
                         | (candidate, ns, match_len) ->
                             let uu___4 =
                               FStar_Compiler_Util.string_of_int match_len in
                             FStar_Compiler_Util.print3 "%s %s %s \n" uu___4
                               ns candidate) uu___2);
                   FStar_Compiler_Util.print_string "#done-ok\n";
                   go line_col filename stack curmod env ts)
              | Pop msg ->
                  (pop env msg;
                   (let uu___2 =
              ...
              | Push (lax, l, c) ->
                  let uu___1 =
                    if
                      (FStar_Compiler_List.length stack) =
                        (FStar_Compiler_List.length ts)
                    then
