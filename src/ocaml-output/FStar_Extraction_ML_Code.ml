...
let (num : Prims.int -> doc) =
  fun i -> let uu___ = FStar_Compiler_Util.string_of_int i in Doc uu___
let (break1 : doc) = text " "
    ...
    fun ns ->
      ...
      then []
        ...
        (let cg_libs = FStar_Options.codegen_libs () in
         let ns_len = FStar_Compiler_List.length ns in
         let found =
           FStar_Compiler_Util.find_map cg_libs
             (fun cg_path ->
                let cg_len = FStar_Compiler_List.length cg_path in
                if (FStar_Compiler_List.length cg_path) < ns_len
                then
                  let uu___1 = FStar_Compiler_Util.first_N cg_len ns in
                  match uu___1 with
                  | (pfx, sfx) ->
                      (if pfx = cg_path
                       then
                         let uu___2 =
                           let uu___3 =
                             ...
                             [uu___4] in
                           FStar_Compiler_List.op_At pfx uu___3 in
...
let (ptsym :
  ...
  fun currentModule ->
    fun mlp ->
      if FStar_Compiler_List.isEmpty (FStar_Pervasives_Native.fst mlp)
                ...
                then "\\n"
                  ...
                  then "\\'"
                    ...
                    then "\\\""
                      ...
                      if FStar_Compiler_Util.is_letter_or_digit uu___
                      then FStar_Compiler_Util.string_of_char uu___
                      else
                        if FStar_Compiler_Util.is_punctuation uu___
                        then FStar_Compiler_Util.string_of_char uu___
                        else
                          if FStar_Compiler_Util.is_symbol uu___
                          then FStar_Compiler_Util.string_of_char uu___
                          else fallback uu___
    ...
    fun outer ->
      fun ty ->
        ...
        | FStar_Extraction_ML_Syntax.MLTY_Var x ->
            let escape_tyvar s =
              if FStar_Compiler_Util.starts_with s "'_"
              then FStar_Compiler_Util.replace_char s 95 117
              else s in
            text (escape_tyvar x)
      ...
      fun e ->
        ...
        | FStar_Extraction_ML_Syntax.MLE_Match (cond, pats) ->
            let cond1 =
              doc_of_expr currentModule (min_op_prec, NonAssoc) cond in
            let pats1 =
              FStar_Compiler_List.map (doc_of_branch currentModule) pats in
            let doc1 =
              ...
              let uu___1 =
                ...
                let uu___3 =
                  let uu___4 =
                    let uu___5 =
                      let uu___6 =
                        FStar_Compiler_List.map (doc_of_branch currentModule)
                          pats in
  ...
  fun currentModule ->
    fun p ->
      fun e1 ->
        fun e2 ->
          let uu___ =
            let uu___1 = as_bin_op p in FStar_Compiler_Option.get uu___1 in
          match uu___ with
          | (uu___1, prio, txt) ->
...
and (doc_of_uniop :
  ...
  fun currentModule ->
    fun p ->
      fun e1 ->
        let uu___ =
          let uu___1 = as_uni_op p in FStar_Compiler_Option.get uu___1 in
        match uu___ with
        | (uu___1, txt) ->
  ...
  fun currentModule ->
    fun pattern ->
          ...
          let doc1 =
            ...
            | (uu___, (FStar_Extraction_ML_Syntax.MLP_Tuple uu___1)::[]) ->
                let uu___2 =
                  let uu___3 =
                    let uu___4 =
                      let uu___5 = FStar_Compiler_List.hd pats in
      ...
      | FStar_Extraction_ML_Syntax.MLP_Tuple ps ->
          let ps1 = FStar_Compiler_List.map (doc_of_pattern currentModule) ps in
          let uu___ = combine (text ", ") ps1 in parens uu___
      | FStar_Extraction_ML_Syntax.MLP_Branch ps ->
          let ps1 = FStar_Compiler_List.map (doc_of_pattern currentModule) ps in
          let ps2 = FStar_Compiler_List.map parens ps1 in
          combine (text " | ") ps2
and (doc_of_branch :
  FStar_Extraction_ML_Syntax.mlsymbol ->
  ...
  fun currentModule ->
    fun uu___ ->
      ...
      | (rec_, top_level, lets) ->
          ...
          let lets1 = FStar_Compiler_List.map for1 lets in
          let lets2 =
  ...
  fun uu___ ->
    ...
    | (lineno, file) ->
        ...
        then empty
          ...
          (let file1 = FStar_Compiler_Util.basename file in
           let uu___3 =
             let uu___4 =
...
let (doc_of_mltydecl :
  ...
  fun currentModule ->
    fun decls ->
        ...
        | { FStar_Extraction_ML_Syntax.tydecl_assumed = uu___1;
            ...
            let forbody body1 =
              ...
              | FStar_Extraction_ML_Syntax.MLTD_Record fields ->
                  ...
                  let uu___3 =
                    let uu___4 = FStar_Compiler_List.map forfield fields in
              ...
              | FStar_Extraction_ML_Syntax.MLTD_DType ctors ->
                  let forctor uu___3 =
                    match uu___3 with
                    | (name, tys) ->
                        let uu___4 = FStar_Compiler_List.split tys in
                        (match uu___4 with
                         | (_names, tys1) ->
                             (match tys1 with
                              | [] -> text name
                  ...
                  let ctors1 = FStar_Compiler_List.map forctor ctors in
                  let ctors2 =
                    FStar_Compiler_List.map (fun d -> reduce1 [text "|"; d])
                      ctors1 in
      ...
      let doc1 = FStar_Compiler_List.map for1 decls in
      let doc2 =
        if (FStar_Compiler_List.length doc1) > Prims.int_zero
        then
          let uu___ =
            let uu___1 =
...
and (doc_of_sig :
  ...
  fun currentModule ->
    fun s ->
      let docs = FStar_Compiler_List.map (doc_of_sig1 currentModule) s in
      let docs1 =
        FStar_Compiler_List.map (fun x -> reduce [x; hardline; hardline])
          docs in
      reduce docs1
let (doc_of_mod1 :
  FStar_Extraction_ML_Syntax.mlsymbol ->
  ...
  fun currentModule ->
    fun m ->
      ...
      | FStar_Extraction_ML_Syntax.MLM_Exn (x, args) ->
          let args1 =
            FStar_Compiler_List.map FStar_Pervasives_Native.snd args in
          let args2 =
            FStar_Compiler_List.map
        ...
        FStar_Compiler_List.map
          (fun x ->
             ...
             (match x with
              ...
              | uu___ -> hardline);
             hardline]) m in
      reduce (FStar_Compiler_List.flatten docs)
let (doc_of_mllib_r :
    ...
    | FStar_Extraction_ML_Syntax.MLLib mllib ->
          ...
          | (x, sigmod, FStar_Extraction_ML_Syntax.MLLib sub) ->
              ...
              let doc1 =
                FStar_Compiler_Option.map
                  ...
                  sigmod in
              let sub1 = FStar_Compiler_List.map for1_sig sub in
              let sub2 =
                FStar_Compiler_List.map
        ...
        and for1_mod istop uu___1 =
          ...
          | (mod_name, sigmod, FStar_Extraction_ML_Syntax.MLLib sub) ->
              ...
              let sub1 = FStar_Compiler_List.map (for1_mod false) sub in
              let sub2 =
                FStar_Compiler_List.map
                      ...
                      let uu___6 =
                        ...
                        (match doc1 with
                         ...
                         | FStar_Pervasives_Native.Some s -> cat s hardline)
                          :: uu___7 in
                      hardline :: uu___6 in
                    FStar_Compiler_List.op_At maybe_open_pervasives uu___5 in
