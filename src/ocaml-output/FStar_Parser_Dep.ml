...
let (check_and_strip_suffix :
  ...
  fun f ->
    ...
    let uu___ =
      FStar_Compiler_List.filter FStar_Compiler_Util.is_some matches in
    match uu___ with
...
let (__proj__Mkdeps__item__parse_results :
  deps -> parsing_data FStar_Compiler_Util.smap) =
  fun projectee ->
    match projectee with
    | { dep_graph; file_system_map; cmd_line_files; all_files;
...
let (deps_try_find :
  ...
  fun uu___ ->
    fun k ->
      match uu___ with | Deps m -> FStar_Compiler_Util.smap_try_find m k
let (deps_add_dep : dependence_graph -> Prims.string -> dep_node -> unit) =
  fun uu___ ->
    fun k ->
      fun v ->
        match uu___ with | Deps m -> FStar_Compiler_Util.smap_add m k v
let (deps_keys : dependence_graph -> Prims.string Prims.list) =
  fun uu___ -> match uu___ with | Deps m -> FStar_Compiler_Util.smap_keys m
let (deps_empty : unit -> dependence_graph) =
  fun uu___ ->
    let uu___1 = FStar_Compiler_Util.smap_create (Prims.of_int (41)) in
    Deps uu___1
let (mk_deps :
  dependence_graph ->
      ...
      file_name Prims.list ->
        file_name Prims.list ->
          module_name Prims.list ->
            parsing_data FStar_Compiler_Util.smap -> deps)
  =
  fun dg ->
    fun fs ->
      fun c ->
...
let (empty_deps : deps) =
  ...
  let uu___1 = FStar_Compiler_Util.smap_create Prims.int_zero in
  let uu___2 = FStar_Compiler_Util.smap_create Prims.int_zero in
...
let (resolve_module_name :
  ...
  fun file_system_map ->
    fun key ->
      let uu___ = FStar_Compiler_Util.smap_try_find file_system_map key in
      match uu___ with
...
let (cache_file_name : Prims.string -> Prims.string) =
  let checked_file_and_exists_flag fn =
    ...
    | FStar_Pervasives_Native.Some path ->
        ...
        ((let uu___2 =
            ((let uu___3 = FStar_Options.dep () in
              ...
              ((Prims.op_Negation
                 ...
                 ||
                 (let uu___3 =
                    FStar_Compiler_Util.paths_to_same_file path
  ...
  let memo1 f x =
    ...
    | FStar_Pervasives_Native.None ->
        let res = f x in (FStar_Compiler_Util.smap_add memo x res; res) in
              ...
              let uu___ =
                (cmd_line_has_impl key) &&
                  (let uu___1 = FStar_Options.dep () in
                   FStar_Compiler_Option.isNone uu___1) in
              if uu___
              then
...
let (build_inclusion_candidates_list :
  ...
  fun uu___ ->
    ...
    let include_directories1 =
      FStar_Compiler_List.map FStar_Compiler_Util.normalize_file_path
        include_directories in
    let include_directories2 =
      FStar_Compiler_List.unique include_directories1 in
    let cwd =
      let uu___1 = FStar_Compiler_Util.getcwd () in
...
let (build_map : Prims.string Prims.list -> files_for_module_name) =
  fun filenames ->
    ...
    let add_entry key full_path =
      let uu___ = FStar_Compiler_Util.smap_try_find map key in
      match uu___ with
      | FStar_Pervasives_Native.Some (intf, impl) ->
          let uu___1 = is_interface full_path in
          if uu___1
          then
            FStar_Compiler_Util.smap_add map key
    ...
    fun last ->
      ...
      let names =
        let uu___ =
          ...
          FStar_Compiler_List.map (fun x -> FStar_Ident.string_of_id x)
            uu___1 in
        FStar_Compiler_List.op_At uu___ suffix in
...
let (hard_coded_dependencies :
  ...
  fun full_filename ->
    let filename = FStar_Compiler_Util.basename full_filename in
    let implicit_module_deps1 =
      FStar_Compiler_List.map (fun l -> (l, Open_module))
    ...
    then []
      ...
      (let uu___2 =
       ...
       match uu___2 with
       | FStar_Pervasives_Native.None ->
           FStar_Compiler_List.op_At implicit_ns_deps1 implicit_module_deps1
       | FStar_Pervasives_Native.Some ns ->
           FStar_Compiler_List.op_At implicit_ns_deps1
             (FStar_Compiler_List.op_At implicit_module_deps1
                [(ns, Open_namespace)]))
      ...
      fun prefix ->
        fun implicit_open ->
          ...
          let suffix_exists mopt =
            ...
            | FStar_Pervasives_Native.Some (intf, impl) ->
                (FStar_Compiler_Util.is_some intf) ||
                  (FStar_Compiler_Util.is_some impl) in
               ...
               fun uu___1 ->
                 ...
                 then
                   ...
                   ((let suffix_filename =
                    ...
                    (let filename =
                       let uu___3 =
                         FStar_Compiler_Util.smap_try_find original_map k in
                       FStar_Compiler_Util.must uu___3 in
                     FStar_Compiler_Util.smap_add working_map suffix filename;
    ...
    fun filename ->
      fun get_parsing_data_from_cache ->
        let from_parsing_data pd original_map1 filename1 =
          let deps1 = FStar_Compiler_Util.mk_ref [] in
          let has_inline_for_extraction = FStar_Compiler_Util.mk_ref false in
          let mo_roots =
          ...
          let set_interface_inlining uu___ =
            ...
            if uu___1
            then
              FStar_Compiler_Effect.op_Colon_Equals has_inline_for_extraction
                true
            else () in
          let add_dep deps2 d =
            let uu___ =
              let uu___1 =
                let uu___2 = FStar_Compiler_Effect.op_Bang deps2 in
                FStar_Compiler_List.existsML (dep_subsumed_by d) uu___2 in
              Prims.op_Negation uu___1 in
            if uu___
            then
              let uu___1 =
                let uu___2 = FStar_Compiler_Effect.op_Bang deps2 in d ::
                  uu___2 in
              FStar_Compiler_Effect.op_Colon_Equals deps2 uu___1
            else () in
          ...
          let record_module_alias ident lid =
            ...
            let uu___ = FStar_Compiler_Util.smap_try_find original_map1 alias in
            match uu___ with
            | FStar_Pervasives_Native.Some deps_of_aliased_module ->
                (FStar_Compiler_Util.smap_add working_map key
...
let (collect_one_cache :
  ...
  =
  let uu___ = FStar_Compiler_Util.smap_create Prims.int_zero in
  FStar_Compiler_Util.mk_ref uu___
let (set_collect_one_cache :
  (dependence Prims.list * dependence Prims.list * Prims.bool)
    FStar_Compiler_Util.smap -> unit)
  =
  fun cache -> FStar_Compiler_Effect.op_Colon_Equals collect_one_cache cache
let (dep_graph_copy : dependence_graph -> dependence_graph) =
  fun dep_graph ->
    let uu___ = dep_graph in
    match uu___ with
    | Deps g -> let uu___1 = FStar_Compiler_Util.smap_copy g in Deps uu___1
let (widen_deps :
  module_name Prims.list ->
    dependence_graph ->
  ...
  fun friends ->
    fun dep_graph ->
      fun file_system_map ->
        fun widened ->
          let widened1 = FStar_Compiler_Util.mk_ref widened in
          let uu___ = dep_graph in
          match uu___ with
          | Deps dg ->
    ...
    fun get_parsing_data_from_cache ->
      ...
      let add_interface_for_inlining l =
        ...
        let uu___ =
          ...
          l1 :: uu___1 in
        FStar_Compiler_Effect.op_Colon_Equals interfaces_needing_inlining
          uu___ in
      let parse_results = FStar_Compiler_Util.smap_create (Prims.of_int (40)) in
      let rec discover_one file_name1 =
        ...
        then
          let uu___1 =
            let uu___2 =
              let uu___3 = FStar_Compiler_Effect.op_Bang collect_one_cache in
              FStar_Compiler_Util.smap_try_find uu___3 file_name1 in
            match uu___2 with
          ...
          | (parsing_data1, (deps1, mo_roots, needs_interface_inlining)) ->
              (if needs_interface_inlining
               ...
               (let deps2 =
                  ...
                  if uu___4
                  then
                    FStar_Compiler_List.op_At deps1
                      [UseInterface module_name1]
                  else deps1 in
                let dep_node1 =
                  let uu___4 = FStar_Compiler_List.unique deps2 in
               ...
               (deps_add_dep dep_graph1 filename
                ...
                (let uu___4 = is_interface filename in
                 ...
                 then
                   ...
                   FStar_Compiler_Util.iter_opt uu___5
                     (fun impl ->
                        if
                          Prims.op_Negation
                            (FStar_Compiler_List.contains impl
                               all_command_line_files)
                        then
                          let uu___6 =
                            let uu___7 =
                              FStar_Compiler_Effect.op_Bang mo_files in
                            impl :: uu___7 in
                          FStar_Compiler_Effect.op_Colon_Equals mo_files
                            uu___6
                        else ())
                 else ())) in
...
let (print_raw : deps -> unit) =
  fun deps1 ->
    ...
    | Deps deps2 ->
        let uu___1 =
          let uu___2 =
            FStar_Compiler_Util.smap_fold deps2
              (fun k ->
                 fun dep_node1 ->
                   fun out ->
                     let uu___3 =
  ...
  fun deps1 ->
    let sort_output_files orig_output_file_map =
      ...
      let remaining_output_files =
        FStar_Compiler_Util.smap_copy orig_output_file_map in
      let visited_other_modules =
        FStar_Compiler_Util.smap_create (Prims.of_int (41)) in
      let should_visit lc_module_name =
        (let uu___ =
           FStar_Compiler_Util.smap_try_find remaining_output_files
             lc_module_name in
         FStar_Compiler_Option.isSome uu___) ||
          (let uu___ =
             FStar_Compiler_Util.smap_try_find visited_other_modules
               lc_module_name in
           FStar_Compiler_Option.isNone uu___) in
      let mark_visiting lc_module_name =
        let ml_file_opt =
          FStar_Compiler_Util.smap_try_find remaining_output_files
            lc_module_name in
        FStar_Compiler_Util.smap_remove remaining_output_files lc_module_name;
        FStar_Compiler_Util.smap_add visited_other_modules lc_module_name
          true;
        ml_file_opt in
      let emit_output_file_opt ml_file_opt =
        ...
        | FStar_Pervasives_Native.Some ml_file ->
            let uu___ =
              let uu___1 = FStar_Compiler_Effect.op_Bang order in ml_file ::
                uu___1 in
            FStar_Compiler_Effect.op_Colon_Equals order uu___ in
      let rec aux uu___ =
        match uu___ with
        | [] -> ()
      ...
      let all_extracted_modules =
        FStar_Compiler_Util.smap_keys orig_output_file_map in
    ...
    let output_file ext fst_file =
      let ml_base_name =
        let uu___ =
          let uu___1 =
            let uu___2 = FStar_Compiler_Util.basename fst_file in
            check_and_strip_suffix uu___2 in
          FStar_Compiler_Option.get uu___1 in
        FStar_Compiler_Util.replace_chars uu___ 46 "_" in
...
let (print_fsmap :
  (Prims.string FStar_Pervasives_Native.option * Prims.string
    FStar_Pervasives_Native.option) FStar_Compiler_Util.smap -> Prims.string)
  =
  fun fsmap ->
    FStar_Compiler_Util.smap_fold fsmap
      (fun k ->
         fun uu___ ->
           fun s ->
             match uu___ with
             | (v0, v1) ->
                 let uu___1 =
                   let uu___2 =
                     FStar_Compiler_Util.format3 "%s -> (%s, %s)" k
                       (FStar_Compiler_Util.dflt "_" v0)
                       (FStar_Compiler_Util.dflt "_" v1) in
                   FStar_String.op_Hat "; " uu___2 in
                 FStar_String.op_Hat s uu___1) ""
