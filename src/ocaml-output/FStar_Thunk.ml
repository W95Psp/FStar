...
let mkv : 'a . 'a -> 'a thunk =
  fun v -> FStar_Compiler_Effect.alloc (FStar_Pervasives.Inr v)
let force : 'a . 'a thunk -> 'a =
  fun t1 ->
    let uu___ = FStar_Compiler_Effect.op_Bang t1 in
    match uu___ with
    | FStar_Pervasives.Inr a1 -> a1
    | FStar_Pervasives.Inl f ->
        let a1 = f () in
        (FStar_Compiler_Effect.op_Colon_Equals t1 (FStar_Pervasives.Inr a1);
         a1)
