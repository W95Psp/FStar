...
let (try_assoc :
  ...
  fun key ->
    fun d ->
      ...
      FStar_Compiler_Util.map_option FStar_Pervasives_Native.snd uu___
...
let (uri_to_path : Prims.string -> Prims.string) =
  fun u ->
    let uu___ =
      let uu___1 =
        FStar_Compiler_Util.substring u (Prims.of_int (9)) (Prims.of_int (3)) in
      uu___1 = "%3A" in
    if uu___
    then
      let uu___1 =
        FStar_Compiler_Util.substring u (Prims.of_int (8)) Prims.int_one in
...
let (path_to_uri : Prims.string -> Prims.string) =
  fun u ->
    let uu___ =
      let uu___1 = FStar_Compiler_Util.char_at u Prims.int_one in uu___1 = 58 in
    if uu___
    then
      let rest =
        let uu___1 = FStar_Compiler_Util.substring_from u (Prims.of_int (2)) in
        FStar_Compiler_Util.replace_char uu___1 92 47 in
      let uu___1 =
